            ...
            if pieces <= 100:
                try:
                    match (calc_method[met]):
                        case 1:
                            graphic, ans = method_right(function, fir_limit, sec_limit, pieces)
                        case 2:
                            graphic, ans = method_left(function, fir_limit, sec_limit, pieces)
                        case 3:
                            graphic, ans = method_parabol(function, fir_limit, sec_limit, pieces)
                        case 4:
                            graphic, ans = method_trap(function, fir_limit, sec_limit, pieces)
                except Exception as e:
                    ans = "Некорректный ввод"
                    hel = None
            else:
